  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        // Регистрация
        if (!username.trim()) {
          setError(t.auth.usernameRequired || 'Имя пользователя обязательно');
          setLoading(false);
          return;
        }
      ...
      if (err.message.includes('already registered')) {
        setError(t.auth.emailAlreadyRegistered || 'Этот email уже зарегистрирован');
      } else if (err.message.includes('Invalid login credentials')) {
        setError(t.auth.invalidCredentials || 'Неверный email или пароль');
      } else if (err.message.includes('занято')) {
        setError(err.message);
      } else {
        setError(err.message || t.auth.error || 'Произошла ошибка');
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          {error && (
            <div className={`rounded-md p-4 ${
              error.includes('успешна') ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'
            }`}>
          ...
          <div>
            ...
            >
              ...
              {t.auth.signInWithGitHub || 'Войти через GitHub'}
            </button>
          </div>

          {!isLogin && (
            <p className="mt-2 text-center text-xs text-gray-500">
              {t.auth.agreementText || 'Регистрируясь, вы соглашаетесь с условиями использования'}
            </p>
          )}
        </form>
        ...
        <div className="text-center">
          <p className="text-sm text-gray-600">
            {t.auth.demoWarning || 'Это продакшен версия с полноценной базой данных'}
          </p>
        </div>
      </div>
    </div>
